          ...
          {projects.map((project, index) => (
            ...
            >
              <div className="h-full p-6 rounded-[18px] bg-secondary/50 backdrop-blur-xl border border-black/[0.08] dark:border-white/[0.08] hover:bg-secondary/80 hover:scale-[1.02] transition-all duration-300">
                ...
                {project.metrics && (
                  <div className="flex gap-3 mb-4">
                    {project.metrics.map((metric, mIndex) => (
                      ...
                      >
                        <div className="text-[12px] text-muted-foreground mb-1">{metric.label}</div>
                        <div className="text-[19px] font-semibold text-primary">{metric.value}</div>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="space-y-2 mb-5">
                  {project.highlights.map((highlight, hIndex) => (
                    <div key={hIndex} className="flex gap-2">
                      ...
                      <p className="text-[13px] leading-[1.47059] text-foreground/60">{highlight}</p>
                    </div>
                  ))}
                </div>
                ...
                <motion.button
                  className="flex items-center gap-1 text-[14px] text-primary font-medium group-hover:gap-2 transition-all"
                  whileHover={{ x: 2 }}
                >
